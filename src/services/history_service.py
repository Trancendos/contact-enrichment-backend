...
class HistoryService:
    ...
    def get_contact_history(self, user_id, contact_id, limit=50):
        ...
        try:
            history = (
                self.db_session.query(ContactHistory)
                .filter_by(user_id=user_id, contact_id=contact_id)
    ...
    def undo_action(self, user_id, history_id):
        ...
        try:
            history_entry = (
                self.db_session.query(ContactHistory)
                .filter_by(id=history_id, user_id=user_id)
                .first()
            )
            ...
            self.log_action(
                ...
                action_type="undo",
                before_data=(
                    json.loads(history_entry.after_data)
                    if history_entry.after_data
                    else None
                ),
    ...
    def get_backups(self, user_id, limit=10):
        ...
        try:
            backups = (
                self.db_session.query(ContactHistory)
                .filter_by(user_id=user_id, contact_id="BACKUP", action_type="backup")
                .order_by(ContactHistory.timestamp.desc())
                .limit(limit)
                .all()
            )
