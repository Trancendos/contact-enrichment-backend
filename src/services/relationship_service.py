...
class RelationshipService:
    ...
    def create_relationship(
        ...
        contact1 = (
            ...
            .filter(Contact.id == contact_id_1, Contact.user_id == self.user_id)
            .first()
        )
        contact2 = (
            self.db.query(Contact)
            .filter(Contact.id == contact_id_2, Contact.user_id == self.user_id)
            .first()
        )

        if not contact1 or not contact2:
            raise ValueError(
                "One or both contacts not found or do not belong to the user."
            )

        # Check for existing relationship (bidirectional)
        existing_relationship = (
            self.db.query(ContactRelationship)
            .filter(
                (
                    (
                        ContactRelationship.contact_id_1 == contact_id_1
                        and ContactRelationship.contact_id_2 == contact_id_2
                    )
                    or (
                        ContactRelationship.contact_id_1 == contact_id_2
                        and ContactRelationship.contact_id_2 == contact_id_1
                    )
                )
    ...
    def delete_relationship(self, relationship_id: int):
        ...
        )  # Assuming to_dict() method exists or will be added
        ...
        contact1 = (
            self.db.query(Contact)
            .filter(Contact.id == contact1_id, Contact.user_id == self.user_id)
            .first()
        )
        contact2 = (
            self.db.query(Contact)
            .filter(Contact.id == contact2_id, Contact.user_id == self.user_id)
            .first()
        )
    ...
    def update_relationship(
        self, relationship_id: int, new_type: str = None, new_description: str = None
    ):
        relationship = (
            self.db.query(ContactRelationship)
            .filter(
        ...
        contact1 = (
            self.db.query(Contact)
            .filter(
                Contact.id == relationship.contact_id_1, Contact.user_id == self.user_id
            )
            .first()
        )
        contact2 = (
            self.db.query(Contact)
            .filter(
                Contact.id == relationship.contact_id_2, Contact.user_id == self.user_id
            )
            .first()
        )
