...
class ContactAIPredictor:
    ...
    def extract_contact_features(self, contact):
        ...
        features = {
            ...
            'email_domains': set([e.get('value', '').split('@')[1] for e in contact.get('emails', []) if '@' in e.get('value', '')]),
            'phone_count': len([p for p in contact.get('phones', []) if p.get('value')]),
            'email_count': len([e for e in contact.get('emails', []) if e.get('value')])
        }
        return features
