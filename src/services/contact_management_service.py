...
class ContactManagementService:
    ...
    def _log_history(
        self, contact_id, action, before_data, after_data, request_info=None
    ):
        self.history_service.log_action(
            ...
            request_info=(
                json.dumps(request_info, default=str) if request_info else None
            ),
        )
    ...
    def create_contact(self, contact_data: dict, request_info=None):
        ...
        self._log_history(
            new_contact.id, "create", {}, new_contact.to_dict(), request_info
        )
    ...
    def update_contact(self, contact_id: int, updated_data: dict, request_info=None):
        contact = (
            ...
            .filter_by(id=contact_id, user_id=self.user_id)
            .first()
        )
        if not contact:
        ...
        self._log_history(
            contact.id, "update", before_data, contact.to_dict(), request_info
        )
    ...
    def split_phone(
        self, original_contact_id: int, phone_to_split: dict, request_info=None
    ):
        original_contact = (
            self.db_session.query(Contact)
            .filter_by(id=original_contact_id, user_id=self.user_id)
            .first()
        )
        ...
        original_contact.phones = [
            p for p in original_contact.phones if p != phone_to_split
        ]
        self.db_session.commit()
        ...
        self._log_history(
            original_contact.id,
            "split_phone_original",
            before_original_data,
            original_contact.to_dict(),
            request_info,
        )
        self._log_history(
            new_contact.id, "split_phone_new", {}, new_contact.to_dict(), request_info
        )

        return {
            "success": True,
            "new_contact": new_contact.to_dict(),
            "updated_original": original_contact.to_dict(),
        }
    ...
    def merge_contacts(self, contact_ids: list[int], request_info=None):
        contacts_to_merge = (
            self.db_session.query(Contact)
            .filter(Contact.id.in_(contact_ids), Contact.user_id == self.user_id)
            .all()
        )
        if not contacts_to_merge or len(contacts_to_merge) < 2:
            return {
                "success": False,
                "error": "At least two contacts are required for merging",
            }
        ...
        for other_contact in other_contacts:
            primary_contact.emails.extend(
                [e for e in other_contact.emails if e not in primary_contact.emails]
            )
            primary_contact.phones.extend(
                [p for p in other_contact.phones if p not in primary_contact.phones]
            )
            primary_contact.tags.extend(
                [t for t in other_contact.tags if t not in primary_contact.tags]
            )
            primary_contact.notes = (primary_contact.notes or "") + (
                f"\nMerged from contact ID {other_contact.id}: {other_contact.notes}"
                if other_contact.notes
                else ""
            )
    ...
    def add_relationship(
        ...
        contact1 = (
            self.db_session.query(Contact)
            .filter_by(id=contact_id_1, user_id=self.user_id)
            .first()
        )
        contact2 = (
            self.db_session.query(Contact)
            .filter_by(id=contact_id_2, user_id=self.user_id)
            .first()
        )
        ...
        existing_rel = (
            self.db_session.query(ContactRelationship)
            .filter(
                (
                ...
                | (
                    (ContactRelationship.contact_id_1 == contact_id_2)
                    & (ContactRelationship.contact_id_2 == contact_id_1)
                )
            )
            .first()
        )
    ...
    def remove_relationship(self, relationship_id: int, request_info=None):
        relationship = (
            self.db_session.query(ContactRelationship)
            .filter_by(id=relationship_id, user_id=self.user_id)
            .first()
        )
        if not relationship:
    ...
    def get_relationships_for_contact(self, contact_id: int):
        ...
        for rel in relationships:
            related_contact_id = (
                rel.contact_id_1 if rel.contact_id_2 == contact_id else rel.contact_id_2
            )
            related_contact = (
                self.db_session.query(Contact).filter_by(id=related_contact_id).first()
            )
            result.append(
                {
                    "id": rel.id,
    ...
    def get_all_relationships(self):
        ...
        for rel in relationships:
            contact1 = (
                self.db_session.query(Contact).filter_by(id=rel.contact_id_1).first()
            )
            contact2 = (
                self.db_session.query(Contact).filter_by(id=rel.contact_id_2).first()
            )
            result.append(
                {
                    "id": rel.id,
                    ...
                    "relationship_type": rel.relationship_type,
                    "contact_1_name": (
                        contact1.full_name if contact1 else "Unknown Contact"
                    ),
                    "contact_2_name": (
                        contact2.full_name if contact2 else "Unknown Contact"
                    ),
                }
            )
        return result
