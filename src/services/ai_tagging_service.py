...
class AITaggingService:
    ...
    def suggest_tags(self, contact_data):
        ...
        try:
            ...
            emails = [
                email.get("value", "") for email in contact_data.get("emails", [])
            ]
            # phones = [
            #     phone.get("value", "") for phone in contact_data.get("phones", [])
            # ]
...
Do not include tags that already exist."""
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful assistant that suggests relevant tags for contact management. Always respond with valid JSON.",
                    },
    ...
    def _fallback_suggest_tags(self, contact_data):
        ...
        if title:
            if any(
                word in title
                for word in ["ceo", "cto", "cfo", "president", "director", "vp"]
            ):
