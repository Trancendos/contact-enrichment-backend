...
def generate_ai_suggestions(contacts, user_id, db_session):
    ...
    for contact in contacts:
        ...
        if contact.get("notes"):
            ...
            nlu_suggestions = nlu_service.generate_suggestions_from_analysis(
                contact, analysis
            )
        ...
        email = next(
            (e.get("value") for e in contact.get("emails", []) if e.get("value")), None
        )
        full_name = contact.get("full_name")

        if company_name or email or full_name:
            explorium_enriched_data = (
                explorium_service_instance.enrich_contact_with_explorium(contact)
            )

            if explorium_enriched_data:
                # Suggest updating company name if Explorium finds a more accurate one
                if (
                    "explorium_business_data" in explorium_enriched_data
                    and explorium_enriched_data["explorium_business_data"].get("name")
                    and explorium_enriched_data["explorium_business_data"][
                        "name"
                    ].lower()
                    != company_name.lower()
                ):
                    suggestions.append(
                        Suggestion(
                            ...
                            suggested_value=explorium_enriched_data[
                                "explorium_business_data"
                            ]["name"],
                            confidence=0.95,
                ...
                if "explorium_prospect_data" in explorium_enriched_data:
                    ...
                    if prospect_data.get("job_title") and prospect_data[
                        "job_title"
                    ] != contact.get("title"):
                        suggestions.append(
                            Suggestion(
                    ...
                    if prospect_data.get("linkedin_url") and prospect_data[
                        "linkedin_url"
                    ] not in [url.get("value") for url in contact.get("url", [])]:
                        suggestions.append(
                            Suggestion(
                                user_id=user_id,
                                contact_id=contact_id,
    ...
    for i, contact1 in enumerate(all_contacts):
        for contact2 in all_contacts[i + 1 :]:
            merge_probability = predictor.calculate_merge_probability(
                contact1.to_dict(), contact2.to_dict()
            )
...
def list_suggestions():
    ...
    return jsonify({"success": True, "suggestions": [s.to_dict() for s in suggestions]})
...
def approve_suggestion(suggestion_id):
    ...
    suggestion = (
        g.db.query(Suggestion).filter_by(id=suggestion_id, user_id=user_id).first()
    )
    ...
    if suggestion.field_name == "action":
        if "split" in suggestion.suggested_value.lower():
            predictor.learn_from_feedback(
                "split", {"confidence": suggestion.confidence}, approved=True
            )
        elif "merge" in suggestion.suggested_value.lower():
            predictor.learn_from_feedback(
                "merge", {"confidence": suggestion.confidence}, approved=True
            )
...
def reject_suggestion(suggestion_id):
    ...
    if suggestion.field_name == "action":
        if "split" in suggestion.suggested_value.lower():
            predictor.learn_from_feedback(
                "split", {"confidence": suggestion.confidence}, approved=False
            )
        elif "merge" in suggestion.suggested_value.lower():
            predictor.learn_from_feedback(
                "merge", {"confidence": suggestion.confidence}, approved=False
            )
...
def bulk_action():
    ...
    suggestions = (
        g.db.query(Suggestion)
        .filter(Suggestion.id.in_(suggestion_ids), Suggestion.user_id == user_id)
        .all()
    )
